    ...
    return (
        <article className="grid gap-4 sm:gap-7 md:grid-cols-2 grid-rows-1 group items-center justify-center">
            <Link href={project.url ? project.url : project.codeUrl!} target="_blank">
                <div className="relative rounded-xl overflow-hidden flex bg-gray-100">
            ...
            <div className="flex flex-col gap-2.5 justify-center py-4">
                <h3 className="font-bold text-3xl text-slate-800">{project.title}</h3>
                ...
                <div className="flex justify-start gap-3">
                    {project.codeUrl && (
                        <Link href={project.codeUrl!} target="_blank" className="text-lg px-4 py-2 bg-slate-900 text-white font-bold rounded-lg">
                            Code
                        </Link>
                    )}
                    {project.url && (
                        <Link href={project.url} target="_blank" className="flex gap-3 group/live hover:gap-6 transition-[gap] items-center text-lg py-2 px-5 bg-slate-900 text-white font-bold rounded-lg">
                            Live
